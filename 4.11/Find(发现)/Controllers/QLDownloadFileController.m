...
- (void)createUI
{
    ...
    UIButton *parseBtn = [QLViewCreateTool createButtonWithFrame:CGRectMake(KScreenSize.width - 110, 80, 100, 30) title:@"暂停下载" target:self sel:@selector(parseDownload)];
...
- (void)btnClickToDownloadFile
{
   ...
   self.progress = [[[NSUserDefaults standardUserDefaults] objectForKey:@"progress"] floatValue];
    if ([fileManage fileExistsAtPath:self.filePath] && self.progress == 1.00){
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    if (self.progress == 1.00) {
        [[NSUserDefaults standardUserDefaults] setObject:@(self.progress) forKey:@"progress"];
...
- (void)deleteDownloadFile
{
    ...
    //进度清零
    UILabel *progressLabel = (UILabel *)[self.view viewWithTag:100];
    [[NSUserDefaults standardUserDefaults]removeObjectForKey:@"progress"];
