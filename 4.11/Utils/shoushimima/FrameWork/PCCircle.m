...
//    CGPathRelease(circlePath);
    ...
    UIImage *image;
    if(self.state == CircleStateNormal)
    {
        image = [UIImage imageNamed:@"gesture_img"];
    }
    else{
        image = [UIImage imageNamed:@"gesture_selectImg"];
    }
    CGContextDrawImage(ctx, CGRectMake(0, 0, rect.size.width, rect.size.height), image.CGImage);
}
