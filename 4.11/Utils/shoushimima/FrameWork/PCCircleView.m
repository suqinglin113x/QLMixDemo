...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            ...
            if (equal){
                // 一致，存储密码
                NSString *ssid = [[NSUserDefaults standardUserDefaults] objectForKey:@"userId"];
                [PCCircleViewConst saveGesture:gesture Key:ssid];
...
{
    NSString *ssid = [[NSUserDefaults standardUserDefaults] objectForKey:@"userId"];
    NSString *password = [PCCircleViewConst getGestureWithKey:ssid];
