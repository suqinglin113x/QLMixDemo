//
//  QLScanViewController.m
//  4.11
//
//  Created by SU on 16/11/8.
...
@implementation QLScanViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"开启闪关灯" style:UIBarButtonItemStylePlain target:self action:@selector(rightBarButtonDidClick)];
...
- (void)createMaskView
{
    QLMaskView *maskView = [[QLMaskView alloc] initWithFrame:CGRectMake(0, 64, KScreenSize.width, KScreenSize.height)];
...
- (void)rightBarButtonDidClick
{
    ...
    if ([device hasFlash] && [device hasTorch]) {
        ...
        if (self.flashOpen) {
            ...
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"关闭闪关灯" style:UIBarButtonItemStylePlain target:self action:@selector(rightBarButtonDidClick)];
        }else{
